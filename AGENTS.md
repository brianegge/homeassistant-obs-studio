...
Custom Home Assistant integration (`obs_websocket`) that monitors OBS Studio via WebSocket v5. It exposes two sensor entities for stream status and service configuration.

## Directory Structure

```
obs_websocket/
├── __init__.py       # Core: OBSConnection, OBSCoordinator, entry setup/teardown
├── config_flow.py    # Config flow: host, port, password validation
...
├── strings.json      # UI localization strings for config flow
...
| `OBSCoordinator` | `__init__.py` | DataUpdateCoordinator with 60s heartbeat and event-driven refresh |
| `OBSWebSocketConfigFlow` | `config_flow.py` | Config flow with connection validation |
| `OBSStreamStatusSensor` | `sensor.py` | Stream state sensor (streaming/reconnecting/idle) |
| `OBSStreamServiceSensor` | `sensor.py` | Stream service type and settings sensor |
...
### Adding a new sensor
1. Define a new `SensorEntity` subclass in `sensor.py`.
2. Add it to the `async_setup_entry` entity list in `sensor.py`.
3. If it needs new data, update `OBSConnection.async_fetch_data()` in `__init__.py`.
...
4. Use the existing coordinator from `hass.data[DOMAIN][entry.entry_id]["coordinator"]`.
...
- Test config flow validation, coordinator updates, and sensor state mapping.
